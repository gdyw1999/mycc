...
**检查项**：
1. Codex 是否已安装（能运行 `Codex --version`）
2. 当前目录是否是 mycc 项目根目录（存在 `AGENTS.md`）
...
**如果失败**：
- Codex 未安装 → 引导用户去 https://docs.anthropic.com/en/docs/Codex 安装
...
| 源文件 | 目标文件 | 说明 |
|--------|----------|------|
| `.Codex/settings.local.json.example` | `.Codex/settings.local.json` | Hooks 配置 |
...
**交互方式**：
> 问用户："你希望我怎么称呼你？（这个名字会出现在 AGENTS.md 里）"
...
**需要替换的文件**：
- `AGENTS.md`：把所有 `{{YOUR_NAME}}` 替换为用户的名字

**执行方式**：
```bash
sed -i '' 's/{{YOUR_NAME}}/用户名字/g' AGENTS.md
```

**检查点**：确认 `AGENTS.md` 中不再包含 `{{YOUR_NAME}}`
...
**验证项**：
1. `.Codex/settings.local.json` 存在且格式正确
...
```bash
# 检查文件存在
ls -la .Codex/settings.local.json
ls -la 0-System/status.md
ls -la 0-System/context.md

# 检查变量已替换
grep "{{YOUR_NAME}}" AGENTS.md || echo "✅ 变量已全部替换"
```
...
### Q: Hooks 没生效
A:
1. 确认 `.Codex/settings.local.json` 存在
2. 重启 Codex
3. 检查文件路径是否正确

### Q: 想重新配置
A:
1. 删除 `.Codex/settings.local.json`
2. 重新运行 `/setup`

### Q: 想改名字
A: 直接编辑 `AGENTS.md`，把名字改成你想要的
