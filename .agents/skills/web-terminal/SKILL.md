...
多标签页 AI 终端，预置 Codex / Codex 标签页，通过浏览器远程访问，手机也能用。
...
```bash
WEB_TERMINAL_TOKEN=mycc node .Codex/skills/web-terminal/scripts/server.mjs
```
...
- **多标签页**：Codex、Codex 两个 AI CLI 标签页切换
...
```bash
WEB_TERMINAL_TABS='[{"id":"Codex","label":"CC","cmd":"Codex","args":["--continue"]},{"id":"aider","label":"Aider","cmd":"aider","args":[]}]' node server.mjs
```

## 脚本路径

`.Codex/skills/web-terminal/scripts/server.mjs`

依赖已安装在 `.Codex/skills/web-terminal/scripts/node_modules/`
