...
node "$PROJECT_ROOT/.Codex/skills/collect/scripts/collect.mjs" --save "$SAVE_DIR"
```
...
- **`token_expired`**（fxb）→ 发飞书红卡通知：
  ```bash
  node "$PROJECT_ROOT/.Codex/skills/tell-me/send.js" "风向标 Token 过期" "API 返回 success: false，请刷新 token。\n操作：跟 cc 说「刷新生财 token」即可自动完成" "red"
  ```
- **`token_expiring_soon`**（fxb，warning 字段）→ 发飞书黄卡预警：
  ```bash
  node "$PROJECT_ROOT/.Codex/skills/tell-me/send.js" "风向标 Token 即将过期" "剩余 {daysLeft} 天，{expireDate} 到期。\n操作：跟 cc 说「刷新生财 token」即可自动完成" "yellow"
  ```
...
1. 读取分析标准：$PROJECT_ROOT/.Codex/skills/collect/analysis/{source}-analyst.md
...
```bash
node "$PROJECT_ROOT/.Codex/skills/tell-me/send.js" \
  "每日简报 | {YYYY-MM-DD}" \
...
只运行部分 collector：
```bash
node "$PROJECT_ROOT/.Codex/skills/collect/scripts/collect.mjs" --sources fxb,gh-trending
```
