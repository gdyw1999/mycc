...
启动 mycc 小程序本地后端，连接网页版/小程序与本地 Codex。

## 环境要求

| 要求 | 说明 |
|------|------|
| Codex | **必须是官方原版**，fork 版本可能不兼容 |
...
>
> 💡 **关于第三方 Codex**：目前仅测试了官方原版，第三方 fork 版本的兼容性支持在规划中。
...
```bash
cd .Codex/skills/mycc/scripts && npm install && cd -
```

### 2. 启动后端

```bash
echo "=== $(date) ===" >> .Codex/skills/mycc/mycc.log && nohup .Codex/skills/mycc/scripts/node_modules/.bin/tsx .Codex/skills/mycc/scripts/src/index.ts start >> .Codex/skills/mycc/mycc.log 2>&1 & disown
```

用 `nohup ... & disown` 让后端完全脱离 CC 进程树，关掉 CC 窗口也不会挂。**不要用 `run_in_background: true`**。日志实时写入 `.Codex/skills/mycc/mycc.log`。

> 代码会自动检测项目根目录（向上查找 `.Codex/` 或 `AGENTS.md`），无需手动指定 cwd。

### 3. 读取连接信息

等待几秒后读取：
```bash
sleep 5 && cat .Codex/skills/mycc/current.json
```
...
- **连接信息**：保存在 `.Codex/skills/mycc/current.json`
...
启动后保存在 `.Codex/skills/mycc/current.json`：
```json
{
  "routeToken": "XXXXXX",
  "pairCode": "XXXXXX",
